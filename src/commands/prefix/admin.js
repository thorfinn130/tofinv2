...
module.exports = {
  ...
  run(message, args) {
    ...
    if (sub === "level") {
      ...
      if (!target || isNaN(level)) return message.reply({ embeds: [warn("Usage", "`,admin level @user <level>`")] });
    ...
    if (sub === "addxp") {
      ...
      if (!target || isNaN(amount)) return message.reply({ embeds: [warn("Usage", "`,admin addxp @user <amount>`")] });
    ...
    if (sub === "resetlevel") {
      const target = message.mentions.users.first();
      if (!target) return message.reply({ embeds: [warn("Usage", "`,admin resetlevel @user`")] });
    ...
    if (sub === "xprate") {
      ...
      if (isNaN(rate)) return message.reply({ embeds: [warn("Usage", "`,admin xprate <amount>` — XP given per message")] });
    ...
    if (sub === "xpcooldown") {
      ...
      if (isNaN(seconds)) return message.reply({ embeds: [warn("Usage", "`,admin xpcooldown <seconds>`")] });
